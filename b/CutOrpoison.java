...
public class CutOrpoison {
    ...
    public static boolean check(int mid,int[] cuts,int[] poisons,int hp){
        int n = Math.min(cuts.length, mid);
        for (int i = 0,j = 1; i < n; i++,j++) {
            hp-=Math.max(cuts[i],(mid-j)*poisons[i]);
            if (hp<=0){
                return true;
            }
        }
